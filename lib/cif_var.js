  ...
  if (status !== FFI_OK) {
    ...
    switch (status) {
      case FFI_BAD_TYPEDEF:
        err = new Error('ffi_prep_cif_var() returned an FFI_BAD_TYPEDEF error');
        err.code = 'FFI_BAD_TYPEDEF';
        err.errno = status;
        break;
      case FFI_BAD_ABI:
        err = new Error('ffi_prep_cif_var() returned an FFI_BAD_ABI error');
        err.code = 'FFI_BAD_ABI';
        err.errno = status;
        break;
      default:
        err = new Error('ffi_prep_cif_var() returned an error: ' + status);
        err.errno = status;
        break;
    }
    throw err;
