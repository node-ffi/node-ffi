...
var dlopen = ffi.ForeignFunction(ffi.Bindings.StaticFunctions.dlopen, voidPtr, [ charPtr, int ])
  ...
  , dlerror = ffi.ForeignFunction(ffi.Bindings.StaticFunctions.dlerror, ffi.Utf8String, [ ])
...
function DynamicLibrary (path, mode) {
  ...
  if (typeof path === 'string') {
    var size = Buffer.byteLength(path) + 1
    var buf = new Buffer(size)
    buf.write(path, 0, size)
    buf[buf.length - 1] = 0
    path = buf
  } else if (!path) {
    path = ref.NULL
...
DynamicLibrary.prototype.get = function (symbol) {
  ...
  if (typeof symbol === 'string') {
    var size = Buffer.byteLength(symbol) + 1
    var buf = new Buffer(size)
    buf.write(symbol, 0, size)
    buf[buf.length - 1] = 0
    symbol = buf
  }
