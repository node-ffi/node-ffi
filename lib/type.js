...
function Type (type) {
  ...
  if (!rtn && type.fields) {
    ...
    // XXX: use "ref-array"?
    var elementsSize = ref.sizeof.pointer * (numFields + 1) // +1 because of the NULL terminator
    var elements = ffi_type.elements = new Buffer(elementsSize)
    for (i = 0; i < numFields; i++) {
      var field = fields[fieldNames[i]]
      var ffi_type_ptr = Type(field.type)
